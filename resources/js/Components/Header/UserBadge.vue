<script setup>

import NavLink from "@/Components/NavLink.vue";
import {ref} from "vue";

const show = ref(false)

</script>

<template>
    <div class="header-left" @click="show = !show">
        <button class="bg-gray-200 px-3 py-2 font-bold rounded">{{
                $page.props.auth.user.full_name ?? $page.props.auth.user.phone
            }}
            <i class="fa fa-angle-down ml-4"></i>
        </button>
        <div :class="show ? 'active z-10' : 'opacity-0 -z-10' " class="popup">
            <div class="flex flex-col justify-between relative">
                <button class="flex justify-between hover:underline px-2 py-2 relative">
                    Notifications
                    <i aria-hidden="true" class="fa fa-bell text-gray-400 notifications"></i>
                </button>
                <NavLink as="button" class="flex justify-between px-2 py-2" href="/logout" method="post">
                    <div>Log Out</div>
                    <i aria-hidden="true" class="fa fa-sign-out text-gray-400 "></i>
                </NavLink>
            </div>
        </div>
    </div>
</template>

<style scoped>
.notifications::after {
    content: '10';
    position: absolute;
    font-size: 12px;
    top: 0;
}
</style>
